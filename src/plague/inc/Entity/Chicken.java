...
public class Chicken extends Animals {
    ...
    protected void incrementStatusTime() {
        StatusTime++;
        if (this.virus.getAnimalIncubationTime() == this.StatusTime)//tester en fonction de l'ï¿½volution de la maladie de l'infectï¿½
        {
            StateChange();
        }
    }
...
=======
    ...
    protected void becomeSick(Viruses v) {
        if (!this.isAlive()) {
            throw new IOError(new Throwable("Le poulet est mort et ne peut changer d'ï¿½tat ! Erreur !"));
        }
        if (this.state.equals(State.HEALTHY)) {
            this.virus = v;
            this.state = State.SICK;
